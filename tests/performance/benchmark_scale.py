...
def run_benchmark_suite(recommender: MovieRecommender) -> Dict:
    ...
    for num_users in test_scales:
        ...
        summary["throughput_req_per_sec"] = (
            num_users / total_time if total_time > 0 else 0
        )
...
def main():
    ...
    print(
        f"   Movies in catalog: {len(recommender.movie_dict) if recommender.movie_dict else 0}"
    )
    ...
    for scale, summary in results.items():
        ...
        if "inference_time" in summary:
            print(
                f"  Mean inference time: {summary['inference_time']['mean_ms']:.2f}ms"
            )
            print(f"  P95 inference time: {summary['inference_time']['p95_ms']:.2f}ms")
            print(f"  Peak memory: {summary['memory']['max_mb']:.2f}MB")
            print(f"  Throughput: {summary['throughput_req_per_sec']:.2f} req/s")
            print(
                f"  Success rate: {summary['successful_runs']}/{summary['total_runs']}"
            )
    ...
    if "error" not in capacity:
        print(
            f"  Baseline throughput: {capacity['baseline_throughput_req_per_sec']:.2f} req/s"
        )
        print(f"  Theoretical max: {capacity['theoretical_max_per_day']:,} req/day")
        print(
            f"  Conservative estimate: {capacity['conservative_estimate_per_day']:,} req/day"
        )
        print(
            f"  Optimistic estimate: {capacity['optimistic_estimate_per_day']:,} req/day"
        )
        print(
            f"  Can handle 1M+/day: {'✅ YES' if capacity['can_handle_1m_per_day'] else '❌ NO'}"
        )
        print(
            f"  Can handle 10M+/day: {'✅ YES' if capacity['can_handle_10m_per_day'] else '❌ NO'}"
        )
