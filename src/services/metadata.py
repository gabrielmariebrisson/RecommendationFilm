...
class MetadataService:
    ...
    def __init__(self, api_key: Optional[str] = None) -> None:
        ...
        if not self.api_key:
            st.warning(
                "Clé API OMDb non trouvée. Les métadonnées de films ne seront pas disponibles."
            )
    ...
    async def get_movie_data_async(
        self, title: str, session: Optional[aiohttp.ClientSession] = None
    ...
    async def get_movies_data_batch(
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            tasks = [
                self.get_movie_data_async(title, session) for title in valid_titles
            ]
...
class TranslationService:
    ...
    def translate(self, text: str, target_language: str) -> str:
        ...
        try:
            ...
            st.debug(
                f"Erreur lors de la traduction de '{text}' vers '{target_language}': {e}"
            )
            return text
