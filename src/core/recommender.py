...
class MovieRecommender:
        ...
        if self.model_registry:
            ...
            if paths:
                ...
                self.scaler_user_path = (
                    paths.get("scaler_user_path") or SCALER_USER_PATH
                )
                self.scaler_item_path = (
                    paths.get("scaler_item_path") or SCALER_ITEM_PATH
                )
                self.scaler_target_path = (
                    paths.get("scaler_target_path") or SCALER_TARGET_PATH
                )
                ...
                self.unique_genres_path = (
                    paths.get("unique_genres_path") or UNIQUE_GENRES_PATH
                )
            else:
                # Fallback vers les chemins par défaut si le registre échoue.
                self.logger.warning(
                    "Failed to load paths from registry, using defaults"
                )
    ...
    def _load_model(_model_path: Path) -> Optional[tf.keras.Model]:
        ...
        try:
            ...
            error_msg = (
                f"Erreur lors du chargement du modèle (fichier non trouvé) : {e}"
            )
            st.error(error_msg)
            return None
    ...
    def initialize(self) -> bool:
        ...
        if self.model_registry:
            self.model_metadata = self.model_registry.load_metadata(
    ...
    def generate_recommendations(
        ...
        try:
            ...
            user_vec: np.ndarray = np.array(
                [[num_ratings, avg_rating, 0] + list(user_prefs.values())]
            )
            ...
            try:
                ...
                sitem_vecs: np.ndarray = self.scaler_item.transform(
                    self.item_vecs_finder[:, 1:]
                )
            except Exception as e:
                self.logger.error(
            ...
            try:
                ...
                suser_tensor = tf.constant(
                    suser_vec_single, dtype=tf.float32
                )  # Shape: (1, features).
                sitem_tensor = tf.constant(
                    sitem_vecs, dtype=tf.float32
                )  # Shape: (num_items, features).
                ...
                suser_broadcasted = tf.broadcast_to(
                    suser_tensor, shape=(num_items, suser_vec_single.shape[1])
                )
                ...
                predictions: np.ndarray = self.model.predict(
                    [suser_broadcasted.numpy(), sitem_tensor.numpy()], verbose=0
                )
                predictions_rescaled: np.ndarray = self.scaler_target.inverse_transform(
                    predictions
                )
            except Exception as e:
    ...
    def health_check(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            num_features = (
                len(self.unique_genres) + 3
            )  # num_ratings, avg_rating, placeholder + genres.
            dummy_user_vec = np.zeros((1, num_features), dtype=np.float32)

            # Répéter pour avoir la même shape que les items.
            num_items = min(
                10, len(self.item_vecs_finder)
            )  # Tester avec 10 items seulement.
            ...
            dummy_predictions = self.model.predict(
                [suser_tensor.numpy(), sitem_tensor.numpy()], verbose=0
            )
